...
let store = new Vuex.Store({
  ...
  mutations:{
    initCarPaneData(state,payload){
      state.carPanelData=payload
    },
    //检测购物车内是否存在对应的物品
    checkCarPaneData(state,payload){
      ...
      if(!state.ball.show){ //小球飞入后才能继续点击
        ...
        carPanelData.forEach((item)=>{
          if(item.id === payload.data.id){
            ...
            if(item.count > item.store){//超出数量限制
              item.count-= payload.count
              state.maxOff = true
              return
            }
            ...
            // 小球飞入
            state.ball.img = payload.data.picture
    ...
    initReceive(state,receiveList){
      state.receiveInfo=receiveList
    },
    ...
    initOrderList(state,orderL){
      state.orderList=orderL
    },
    submitOrder(state,orderData){//提交订单
      state.orderList.unshift(orderData) //添加一条订单

/*      if(orderData.goodsData[0]){//现在购买 不清空购物车
        state.buyNowData = []
        return
      }*/
