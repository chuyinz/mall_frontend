            ...
            <div class="address-common-table js-multiple-address-panel">
              <ul class="address-item-list clear js-address-item-list">
                <li
                  ...
                  :class="{' selected-address-item': index == receiveIndex}"
                  v-for="receive,index in receiveInfo"
                  @click = "changeReceive(index)"
                >
                  <div class="address-item">
                    <div class="name-section">  {{receive.receiver}}  </div>
                    <div class="mobile-section">手机号:{{receive.phone}}</div>
                    <div class="detail-section"> <!--{{receive.province}} {{receive.city}} {{receive.county}}<br> {{receive.add}}-->地址{{index+1}}：{{receive.address}} </div>
                  </div>
                ...
                <li class="add-address-item js-add-address" @click = "closePop(true)">
                  <p>使用新地址</p>
                </li>
              </ul>
            </div>
      ...
      <div class="gray-box">
        ...
        <div class="box-inner ui-goods-cart">
          ...
          <div class="cart-table">
            <div class="cart-group js-cart-group">
              <div class="cart-items" v-for="item,index in goodsCheckedData">
                <div class="items-thumb">
                  <a href="javascript:;" target="_blank"><img :src="picUrl+item.picture"></a>
                </div>
                <div class="name hide-row">
                  <div class="name-cell">
                    <a href="javascript:;" :title="item.name" target="_blank">{{item.name }}</a>
                  </div>
                </div>
                <div class="subtotal">
        ...
        <div class="box-inner">
          <div class="order-discount-line">
            <p> 商品总计： <span>¥ {{goodsCheckedPrice}}.00</span> </p>
<!--            <p> 运费： <span>+ ¥ {{freight}}.00</span> </p>-->
        ...
        <div class="box-inner">
          <div class="last-payment clear">
            ...
            <span class="prices fn-right">应付金额： <em>¥ {{goodsCheckedPrice }}.00</em></span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  data(){
    return {
      receiveIndex:0, //当前选择的收货地址
      showPop:false, //是否显示填写收货信息组件
    }
  },
  components: {
    addressPop,
  ...
  created() {
    this.$axios.get('/customer/getAddress')
            .then(r=>{
              let temp=r.data;
              ...
              this.$store.commit('initReceive',temp)
            })
  ...
  methods: {
    changeReceive(index) { //点击切换收货地址
      this.receiveIndex = index
    },
    closePop(boolean){ // 关闭或显示 收货信息填写
      this.showPop = boolean
    },
    ...
    submitOrder(){ //提交订单
      ...
      {
        GoodsIdList[i]=goods[i].id.toString();
        GoodsNumberList[i]=goods[i].count;
      }

      this.$axios.put('/cart/deleteMultiGoods',{
      ...
      this.$axios.post('/order/createOrder',{
        ...
        customerAddressId:receiveFinal.id,
        goodsIdList:GoodsIdList,
        ...
        this.$router.push({name:'account'})//跳转支付
      })
              .catch(error=>{
                this.$message({
                  type: 'success',
