...
export default {
  ...
  created(){
    this.$axios.get('/customer/getAddress')
            .then(response=>{
              this.$store.state.receiveInfo=response.data;
            })
            .catch(error=>{
              console.log(error);
            })
  ...
  methods: {
    ...
    deleteReceive(receive,index){ //删除收货地址

      this.$axios.delete('/customer/deleteAddress/'+receive.id)
              .then(res=>{
