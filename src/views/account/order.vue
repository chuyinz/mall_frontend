          ...
          <div class="box-inner order-cart order-list-cart clear" v-for="order,index in orderList">
            <div class="gray-sub-title cart-title">
              ...
              <router-link
                tag="span"
                class="order-detail"
                :to="{ 'name': 'payment', 'query':{'id':order.id}}"
              >
                <a>查看详情&gt;</a>
              </router-link>
            ...
            <div class="cart">
              <div class="cart-items clear" v-for="item,index in order.goodsList">
...
<!--                    <a href="javascript:;" target="_blank"><img :src="picUrl+item.picture"></a>-->
            ...
            <div class="prod-operation">
              <div class="total">¥ {{sumPrice(order.goodsList)}}.00</div>
              <router-link v-if="order.state===1"  class="status" tag="div" :to="{ name: 'payment', query:{id:order.id}}">
                <a class="blue-small-btn js-payment-order">现在付款</a>
              </router-link>
              <div class="status" v-if="order.state===4" @click="confirmOrder(order.id)">
                <a class="blue-small-btn js-payment-order">确认收货</a>
              </div>
<!--              <router-link v-if="order.state===4"  class="status" tag="div" :to="{ name: 'payment', query:{id:order.orderId}}">
                <a class="blue-small-btn js-payment-order">确认收货</a>
              </router-link>-->
              <div class="status"v-if="order.state===3">
                <span>等待发货</span>
              </div>
              <div class="status"v-if="order.state===5">
                <span>已完成</span>
              </div>
            </div>
          </div>
...
export default {
  ...
  created(){
    ...
    this.$axios.get('/seller/getMyCostomerOrder')
            .then(res=>{
              this.$store.commit('initOrderList',res.data)
            }).catch(error=>{
      console.log(error)
      this.$message({
        type:'error',
