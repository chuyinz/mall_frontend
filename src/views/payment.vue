        ...
        <div class="confirm-goods-table" v-for="item,index in orderInfo.goodsData">
            <div class="cart-items clear">
                <div class="name fn-left hide-row">
                    <div class="name-cell">
                        <a href="javascript:;" :title="item.name" target="_blank">{{item.name}}</a><br>
                    </div>
                </div>
    ...
    <div id="payment">
        <div class="content page-order-payment">
            <div class="gray-box clear">
                ...
                <div class="box-inner order-info" v-if="orderInfo.state===1">
                    <h3>提交订单成功</h3>
                    <p class="payment-detail">请在 <span>24 小时内</span>完成支付，超时订单将自动取消。</p>
                    <p class="payment-detail">我们将在您完成支付后的 72 小时内发货</p>
                </div>
                <div class="box-inner order-info" v-else-if="orderInfo.state===3">
                    <h3>订单已下达，等待发货</h3>
                </div>
                <div class="box-inner order-info" v-else-if="orderInfo.state===4">
                    <h3>商品已送出，确认收货</h3>
                </div>
                <div class="box-inner order-info" v-else-if="orderInfo.state===5">
                    <h3>订单已完成，谢谢惠顾</h3>
                </div>

                <div class="box-inner payment-checkout-panel clear">
                <span v-if="orderInfo.state===1" class="jianguo-blue-main-btn big-main-btn js-payment-order"@click="orderGetPay(orderInfo.id,orderInfo.goodsList)">
                    <a>现在支付</a>
                </span>
                    <span v-else-if="orderInfo.state===3" class="is-pay">等待发货</span>
                    <span v-else-if="orderInfo.state===4" class="jianguo-blue-main-btn big-main-btn js-payment-order"@click="confirmOrder(orderInfo.id)">
                    <a>确认收货</a>
                   </span>
                    <span v-else class="is-pay">已完成</span>
                    <span class="prices"> 应付金额：   <em><span>¥ </span>{{sumPrice(orderInfo.goodsList)}}.00</em>   </span>
                </div>
            </div>
            <div class="confirm-detail">
                <div class="info-title">收货信息</div>
                <p class="info-detail">姓名：{{orderInfo.receiver}}</p>
                <p class="info-detail">联系电话：{{orderInfo.phone}}</p>  <p class="info-detail">详细地址：{{orderInfo.address}}</p>
            </div>
            <div class="confirm-table-title clear">
                <span class="name fn-left">商品信息</span>
                <span class="subtotal fn-right">小计</span>
            ...
            <div class="confirm-goods-table" v-for="item,index in orderInfo.goodsList">
                <div class="cart-items clear">
                    <div class="name fn-left hide-row">
                        <div class="name-cell">
                            <a href="javascript:;" :title="item.name" target="_blank">{{item.name}}</a><br>
            ...
            <div class="order-discount-line">
                <p> 商品总计： <span>¥ {{sumPrice(orderInfo.goodsList)}}.00</span> </p>
            </div>
        </div>
    </div>
</template>
<script>
    export default {
        ...
        computed:{
            orderInfo(){ //要付款的订单
                let orderInfo =this.$store.state.orderList.filter((order)=>{
                    return Number(order.id) === Number(this.$route.query.id)
                })[0]
                return orderInfo
            }
        },
        methods: {
            confirmOrder(orderId){
                this.$axios.put('/order/confirmOrder',{"orderId":orderId})
                    .then(response=>{
                        this.$store.commit('orderGetConfirm',orderId)
                    }).catch(error=>{
                    this.$message({
                        type:'error',
                        message:'确认收货失败！'
            ...
            sumPrice(goodsList){
                var sum=0;
                goodsList.forEach((goods)=>{sum+=(goods.price*goods.num)})
                return sum
            },
            orderGetPay(id,goodsList) {//支付
                var sum=0;
                goodsList.forEach((goods)=>{sum+=(goods.price*goods.num)})

                let result = window.prompt('你应该支付:￥'+ sum+ '元')
                if (sum === Number(result)){
                    this.$axios.put('/order/payOrder',{orderId:id})
                        .then(response=>{
                            this.$message({
                                type:'success',
                                message:'支付成功'
