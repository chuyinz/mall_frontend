...
/*    setDefault(){//设为默认地址
    ...
    checkCanSave(){//检测是否能保存
      if(this.receive.name.trim()&&!this.phoneErr&&this.receive.phone.trim()&&this.receive.add.trim()){
        this.canSave = true
      }else{
        this.canSave = false
      }
    ...
    saveReceiveHandler(){//保存新的收货地址
      if(this.canSave){
        this.$emit('save-receive',{
            receiver:this.receive.name,
            phone:this.receive.phone,
            address:this.receive.add
        })
          //上传到数据库
          this.$axios.post('/customer/addAddress',{
              receiver:this.receive.name,
              phone:this.receive.phone,
              address:this.receive.add
          }).then(response=>{
